...
5. **Say what the address opens.** The bare address, no option, is the normal door: it boots the launch page on the default content. A chronicle once begun writes its whole address into the bar — `content`, `region`, `schedule`, `deck`, `seed` — so that address replays it. Naming a deck opens a chronicle straight, but only within the content named: a stand-in deck needs `content=stand-in` beside it (`?content=stand-in&deck=PH_Deck`), since the default content holds none.

...
- **A drag does not resolve under a window** — a drag held when a window rises still resolves when it is let go, moving the unit or playing the no-aim card under a menu that covers most of the gameplay; having the gameplay resolve the action while the menu is up is not good design.

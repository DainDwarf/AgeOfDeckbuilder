...
- `INTERFACE.md` _What stands over what_, a new paragraph after the tooltip's: "The **small card** a name raises stands where a tooltip does, on the surface it was raised from, and a scrim rising takes it down as it does a tooltip. Cards shown large stand where the one does, however many stand in the row."
...
- The small card: raised after the tooltip's rest on the name, on the surface that raised it — the `ui` scene for a name on a hand card, the overlay for a name on a window's card, on the card shown large or on a small card standing there — on the stratum of its own each of the two scenes holds; a face of its own, its names live; a chain of them stays while the pointer is on any of its later members and goes down from the first one the pointer has left, after the tooltip's handover; a render of what raised it, a press that raises or takes down a window, a scrim rising over its scene and the card shown large take the chain down.
...
- A row of cards shown large stands over the deal window as the one card does today, and the menu opening over it leaves the whole row standing under its scrim, as it leaves the one card, and closes back onto it.
- A wheel notch over a standing small card scrolls the grid under it, as a wheel notch anywhere over the scrim does, and the small card stays where it stands until the pointer leaves it; the name that raised it moving out from under it takes nothing down.
...
- A grid's frame zone and the grid's cards stand on the overlay's `carried` stratum, and the frame is added to the scene before them, so Phaser hands the pointer to a zone inside a card before the frame sees it; the grid's cards find a press by geometry through the frame, never by a hit on the face. A zone inside a grid card would take the frame's scroll drag, its fling and its presses with it wherever a name lies, which is why a grid lays none and finds a name by the geometry it finds a card by.
- The browse's grid is clipped by a mask on its own container and nothing outside that container is touched: a small card raised from a grid card goes on the overlay's small-card stratum, never inside the grid's container, and is excluded from nothing.
...
- A scrim rising over a scene is the pointer leaving it, and `onHover` in `src/ui/design-space.ts` is what already hears that, the move a scene above withheld and the pointer leaving the canvas: a chain that keeps its own hover on raw over and out events outlives all three. The chronicle scene tells the `ui` and `map` scenes' input plugins that a scrim covers them, and the overlay's plugin nothing, since nothing on the overlay has hovered until now: the menu's scrim rising has to reach the overlay's input plugin too, or a chain there and a name's zone on the card shown large stand under the menu. The overlay's own scrim is not a cover to the overlay: it rises with something the overlay raises, and that raise's wipe takes the chain down.
- Strata are Layers and never depth numbers: the small card's stratum is one more Layer in the `ui` scene's table in `src/ui/chronicle-scene.ts`, between the note's and the tooltip's, and one more in the overlay's `Strata` in `src/ui/overlay-scene.ts`, above the note's; the overlay gets no tooltip stratum, nothing raising a tooltip there. `docs/INTERFACE.md` _What stands over what_ says where the small card stands. The row of cards shown large stands on the overlay's `carried` stratum in add order, side by side, and needs no depth.
...
- `e2e/chronicle-screen.ts`'s `named` finds the first object of a name across the running scenes and their layers, so a zone named the same on every face is found on whichever face comes first; the spec needs the zone on a given face, which the helper reads as a child of the named face.
...
3. `src/ui/card-face.ts`: where every name lies answered back, and the live zone the card shown large and a small card lay from it, the hover and the right click handed to whoever drew the face; `src/ui/chronicle-scene.ts` and `src/ui/overlay-scene.ts`: the small card's stratum on each scene, and the menu's cover reaching the overlay; the small card, its chain, its rest and handover, wherever the implementer lands it, one widget instantiated into whichever scene asked; `src/ui/hand.ts`, `src/ui/overlay.ts` and `src/ui/chronicle-scene.ts` answering the hover and the right click on every surface, the hand and a window's grid reading the geometry and the hand holding a card lifted while a chain off it stands, the row of cards shown large in the overlay. `npm run check` passes.

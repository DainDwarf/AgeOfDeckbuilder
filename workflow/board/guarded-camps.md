...
- An action spent spends the move points the unit has left, so a warrior that kills a guard stands where it struck from until the next turn; a test or spec that walks a unit onto the camp it just emptied waits a turn for it.

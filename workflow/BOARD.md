...
- **One catalogue** — `src/content/` exports one catalogue, no list of them and no lookup by version; the launch page has no content row and the boot reads no `content` key; the save is read against the one catalogue; its version is `1`; no file outside `src/content/` imports the age's module. Doc-impact: `docs/INTERFACE.md`. [board/one-catalogue.md](board/one-catalogue.md)

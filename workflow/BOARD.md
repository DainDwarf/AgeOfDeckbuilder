...
- **A staleness pass over the tests** — the rules and content tests carry no fixture piece that no longer acts, both coherence tests ask every script of their catalogue its move and its attack on a settled launch, and `text()` refuses an unfilled placeholder so every spec that walks a placeholder surface proves its filling; `npm test` green. Doc-impact: none. [board/staleness-pass.md](board/staleness-pass.md)

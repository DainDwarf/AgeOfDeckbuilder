...
- **The window spec's claim wait times out on CI** — `e2e/window.spec.ts` "the design space follows a window that grows after boot" timed out twice on CI at `claimFree`'s wait for the held tile, green locally and on a re-run; it may be intermittent.

...
- **Split the map out of CHRONICLE.md** — "The map" has grown into a document of its own, the layers, terraforming, the generator, the region, the river and the border: a `docs/MAP.md` with the generator as its own section, and level-3 seams inside "Events and the capstone".
- **A staleness pass over the tests** — fixtures carrying pieces that no longer act, like the worker at `LURE` left after the fixture's own script took over from the default one; and the nomadic and stand-in coherence tests, which ask their scripts nothing.

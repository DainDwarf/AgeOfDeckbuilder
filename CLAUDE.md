...
| `npm run e2e` | The whole Playwright/Chromium suite, starting the dev server itself; this is the CI check on push to main, and a hook refuses it from a session. A session runs one spec it has touched: `npx playwright test e2e/<spec>.spec.ts`. |

...
- **An answer's landing resolves as stages** — an answer's `lands` returns a bare chronicle, so the whole landing collapses into one opaque `events` stage whatever it did — Wildfire's burns, Make room's camp and raid, the herd's charting; giving the landing stages of its own, `charted{tile}` among them beside `move` and `camp-enter`, lets `charting` do the snapshot writing again and deletes `carriedOver`.

...
| **timeline** | One chronicle's roll of its schedule, from a generator of its own: the turn the next deal is due on, and the capstone's turn; the event is drawn on its due turn. Internal: the player is never shown it, and learns each event when it lands. | forecast, agenda, itinerary |

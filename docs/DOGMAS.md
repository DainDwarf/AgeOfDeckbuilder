...
- **Stay at discussion altitude when the user is discussing.** Never author a full spec in one sweep; check `DESIGN.md` and `BOARD.md` first — much is pre-decided.
...
- **Design is the spec.** Code that disagrees is wrong. A problem found in a design the user chose is reported with its cause — numbers where there are numbers — and the turn ends. "Go ahead" authorises executing the agreed design, not revisiting it. `DESIGN.md` is never edited down to match an implementation.

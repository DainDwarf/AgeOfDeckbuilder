...
- **A fractional quantity is fixed-point.** A rules value that needs fractions over a known, narrow range counts in integers of its smallest unit, with one named constant giving the scale and one formatter reading it back as a decimal at the screen. Floating point is for a range genuinely wide enough to need it. Why: repeated sums and `<=` on floats drift, and a rule decided by a comparison has to be exact.

...
it selects and plays it. A card that aims stays selected, being aimed. A card the city cannot pay
for comes home under the refusal note.
...
**The back key walks back one step per press**: a window or a card shown large, the inspection,
the selection — a card being aimed is let go of as the selection it is — city mode, then the menu.
...
- **The hand selects by click** — a left click selects any card of the hand, payable or not, and a
  ...
  selected; a left click on anything not of the aimed card's target kind lets it go and lands as on

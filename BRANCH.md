...
The groups: `played` over the cost, the card leaving the hand and the effect's changes; one `strike` per hazard; `income` over one `stock` per tile worked; `grow`; `turn` over the tick and one `refreshed` per unit; `enemy-phase` over each enemy's `move` and its `attack` groups, an `attack` carrying attacker and target over the action spent and the target damaged; `camp-capture`; `capstone`; `deal`; `answer`; `reward`; `claim`; `refused` and `runtime-error` holding nothing. `no-deal` is gone: it is a `rolled` with nothing dealt after it. The fall is an `ended` on the first change that leaves the population at nought, and nothing after it is computed. The sight's charting rides on every change, as it does on every stage today: it is derived.

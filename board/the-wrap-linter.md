...
`docs/DOGMAS.md` → *Stack*, the lint row becomes:

> | Lint and format | Biome for code, one `biome.json`; Prettier for markdown, one `.prettierrc` |

`CLAUDE.md` → *Commands*, the lint row becomes and a fmt row follows it:
...
**Plan:**
1. The ship session, before spawning the implementer: run `npx prettier@3.9.6 --prose-wrap never --write "**/*.md"` at the root, confirm `git status` lists only markdown files and none under `dist/`, and commit that alone as `Prep: the docs unwrapped`. The reviewer then reads a mechanism diff, not two thousand lines of joined paragraphs.

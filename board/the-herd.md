...
1. `src/rules/state.ts`, `src/rules/sight.ts`, `src/rules/chronicle.ts` — the chronicle carries the tiles a command's landing put in sight, plain data every `Chronicle` literal holds; what is in sight counts them, the map is charted through them, and they are dropped as a command that is not refused begins. One door adds one.
2. `src/rules/schedule.ts`, beside the fire — two mechanisms, each standing on its own: the candidates near the city for a named feature with the predicate over them, and the deal of that feature onto one candidate drawn from the chronicle's generator. Both name a feature and a distance, never game or 4. Putting a tile in sight belongs to the step above and stays apart from them: _Follow it_ is what wants a deal and a sighting together, and composes the two in its own closure.
3. `src/rules/fixtures.ts` — one fixture event whose need is the predicate and whose answer composes those same two mechanisms with `PH_Fertile` on a distance of the fixture's own, beside a flat free answer that does nothing.
...
5. `src/content/nomadic.ts` — the event `herd`, its need the predicate and `follow-it` composing the deal and the sighting with `game` and Wildfire's distance, `hunt-it` a flat gain of 4 food; its entry in the nomadic schedule at a third. `src/ui/text.ts` — the entries above. No gameplay test on the content.

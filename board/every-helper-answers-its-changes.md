...
3. `src/rules/chronicle.ts`: the `played` group, one `strike` per hazard staged by the end of turn, `chartedOn` over the new set, the cycle's callers reading the chronicle off the helpers they call. Leaves `npm run check` passing on the rules.
...
5. The rules tests: the assertions on the table's grain, and the new ones on the fixture — the `played` group's children on a free card, a costed card and a card that leaves the chronicle; the settle's five; one `strike` per hazard with its changes, holding nothing at a stock already at nought, and the population change a strike takes no longer dropped. Leaves `npm test` passing.
